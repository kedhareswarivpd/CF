          ...
          {industries.map((ind, i) => (
            <Reveal key={ind.title} from="zoom" delay={i * 80}>
              <div className="bg-white dark:bg-dark-surface border border-outline-variant dark:border-dark-outline-variant rounded-lg p-stack-lg hover:shadow-card-hover transition-all hover:-translate-y-1 flex flex-col h-full">
                ...
                <h3 className="font-display text-headline-sm text-black dark:text-white mb-3">{ind.title}</h3>
                <p className="text-body-md text-black dark:text-white mb-4 flex-1">{ind.description}</p>
                <div className="mb-4">
                  <h4 className="font-label-caps text-label-caps uppercase text-black dark:text-white mb-2">Key Challenges</h4>
